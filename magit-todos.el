...
(defvar magit-todos-section-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "b" #'magit-todos-toggle-branch-diff)
...
(defcustom magit-todos-show-branch-items nil
  "Show branch-local task items."
  :type 'boolean)
...
(defun magit-todos-toggle-branch-diff (global-p)
  "Toggle between branch-local tasks and regular task list.
With prefix, toggle globally; otherwise, toggle in current Magit
buffer."
  (interactive "P")
  (if global-p
      (custom-set-default 'magit-todos-show-branch-items (not (default-value magit-todos-show-branch-items)))
    (setq-local magit-todos-show-branch-items (not (buffer-local-value 'magit-todos-show-branch-items (current-buffer)))))
  (magit-todos-update))
...
(cl-defun magit-todos--git-diff-callback (&key magit-status-buffer results-regexp search-regexp-elisp process heading
  ...
  (cl-macrolet ((next-diff () `(re-search-forward (rx bol "diff --git ") nil t))
    ...
    (with-current-buffer (process-buffer process)
      (goto-char (point-min))
      (let (items filename hunk-end line-number)
        (while (next-diff)
...
This function should be called from inside a ‘magit-status’ buffer."
  ...
  (when magit-todos-show-branch-items
...
(magit-todos-defscanner "git diff"
  ...
  :test t
  :command (list "git" "--no-pager" "diff" "--no-color" "-U0" "master...HEAD")
